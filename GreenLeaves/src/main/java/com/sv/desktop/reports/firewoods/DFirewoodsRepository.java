...
public interface DFirewoodsRepository extends JpaRepository<TFireWood, Integer> {
    ...
    @Query(value = "select \n"
            ...
            + "where sup.index_no=t.supplier and t_fire_wood.date between :fromDate and :toDate) as qty,\n"
            + "(select sum(t_fire_wood.amount) from t_fire_wood \n"
            + "left join m_suplier sup on sup.index_no=t_fire_wood.supplier\n"
            + "where sup.index_no=t.supplier and t_fire_wood.date between :fromDate and :toDate) as amount,\n"
    ...
    @Query(value = "select \n"
            ...
            + "t_fire_wood.date between :fromDate and :toDate) as qty,\n"
            + "(select ifnull(sum(t_fire_wood.amount),0.00) from t_fire_wood where t_fire_wood.branch=m.index_no and\n"
            + "t_fire_wood.date between :fromDate and :toDate) as amount\n"
            + "from\n"
            + "m_branch m\n"
            + "left join\n"
    ...
    @Query(value = "select \n"
            ...
            + "where t.index_no=t_fire_wood.firewood_unloard_point and t_fire_wood.date between :fromDate and :toDate) as qty\n"
            + "from\n"
    ...
    @Query(value = "select\n"
            ...
            + "and t_fire_wood.date between :fromDate and :toDate\n"
